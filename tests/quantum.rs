...
fn get_param(param_str: &str) -> (Vec<u32>, Vec<u32>, bool) {
    ...
    if param_list.len() < 1 || param_list.len() > 3 {
        ...
        let occ_list = parse_string_list::<u32>(param_list[0], ",");
        let ctrl_list: Vec<u32>= vec![];
        let dagger = false;
        (occ_list, ctrl_list, dagger)
    } else if param_list.len() == 2 {
        let occ_list = parse_string_list::<u32>(param_list[0], ",");
        let ctrl_list = parse_string_list::<u32>(param_list[1], ",");
        let dagger = false;
        (occ_list, ctrl_list, dagger)
    } else {
        let occ_list = parse_string_list::<u32>(param_list[0], ",");
        let ctrl_list = parse_string_list::<u32>(param_list[1], ",");
        let dagger = check_dagger(param_list[2]);
        (occ_list, ctrl_list, dagger)
    }
}
